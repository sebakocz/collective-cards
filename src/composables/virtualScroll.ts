...
export const useVirtualScroll = (inputList: ComputedRef<any[]>) => {
    ...
    const setRunesPerRow = () => {
        ...
        runesPerRow.value = Math.floor(
            scrollContainer.value?.clientWidth / cardDisplayWidth
        )
    }
    ...
    const setBatchSize = () => {
        ...
        batchSize.value =
            runesPerRow.value *
            (Math.ceil(viewContainer.value?.clientHeight / cardDisplayHeight) +
                bufferSize)
    }
    ...
    const startIndex = computed(() =>
        Math.max(
            0,
            Math.floor(scrollTop.value / cardDisplayHeight) *
                runesPerRow.value -
    ...
    const containerStyle = computed(() => ({
        ...
        paddingTop: `${
            (startIndex.value / runesPerRow.value) * cardDisplayHeight
        }px`,
    }))
